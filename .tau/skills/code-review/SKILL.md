...
Review the affected end state, not only whether the patch works locally. With the current requirements and known constraints, would we choose these boundaries, data flow, and contracts if designing the area today? Examine new exceptions, second sources of truth, duplicate paths, and indirect coordination for an underlying ownership or contract problem. A larger coherent fix can be proportionate, but architectural preference alone is not a finding: establish a concrete failure or maintenance risk introduced by the change. Unrelated pre-existing architecture remains out of scope.

Verify external facts only when they materially affect a potential finding. Start with version-matched source, metadata, or a first-party API. Use open-web research only for a specific unresolved question, prefer primary sources, and stop once there is enough evidence. Do not turn the review into a general upstream audit; report a required fact that remains unavailable as a gap rather than a proven finding.
