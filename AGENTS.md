...
- **Session history** (`src/core/history/`, `src/history/worker/`, `src/core/tools/history.ts`): Snapshot-independent flat active transcripts, immutable creation attributes, machine-local SQLite search/read and durable remote outbox, optional API-key-protected Cloudflare D1/Workers AI service deployed through `tau history`, and the global read-only `history` code-mode tool
