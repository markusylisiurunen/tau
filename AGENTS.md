...
- **AgentRuntime** (`src/core/agent/agent_runtime.ts`): Canonical stateful conversation runtime shared by main sessions, background subagents, and ephemeral threads; owns model/tool subturns, streaming, retries, recovery, compaction, steering, interruption, and durable agent state
...
- **ChatRuntime** (`src/core/runtime/chat_runtime.ts`): Main-session adapter that resolves prompts and fully bound tools into an `AgentSpec`, owns the main `AgentRuntime`, and forwards child-agent supervision events
