...
- **Code mode** (`src/core/tools/code_mode.ts`, `src/core/tools/web.ts`, `src/core/tools/web_discovery.ts`, `src/core/static/code_mode/web/`): Generic code-tool lifecycle plus the Exa-backed `web` implementation. Generated JavaScript runs in a host-owned SES compartment with only bounded `web`, `docs`, and console capabilities; the trusted parent retains provider credentials and services Exa-backed search/fetch, while keyless discovery probes run through the session execution environment's generic Node-script backend. Tool results contain stdout/stderr rather than JavaScript return values
