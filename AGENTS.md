...
- **Bash (assistant)**: Leave `maxOutputTokens` unset in most cases and prefer more scoped commands over larger output. By default, outputs above 8,192 tokens are gated to a 2,048-token preview. With experimental `bashOutputGatekeeper` config enabled, outputs at `<= 4096` tokens pass through directly, outputs between `4097` and `12288` tokens are reviewed by the configured gatekeeper model, and outputs above `12288` tokens are always gated to a 2,048-token preview. If `maxOutputTokens` is set explicitly, Tau skips the gatekeeper and respects that token budget directly. Up to 16,384 may be used autonomously; up to 65,536 only when the user explicitly requests it.
...
- **Global**: `~/.config/tau/config.json` (API keys, `defaultPersona`, `defaultRisk`, `disableBuiltinPersonas`, `disableBuiltinThemes`, `defaultTheme`, `bashCommands`, `agentContextFiles`, `sandbox`, `subagents`, `bashOutputGatekeeper`, `modelSystemNotices`, `async`). This level is only included when cwd is inside home.
  ...
  - `bashOutputGatekeeper` (optional): Experimental bash-output gating config. `bashOutputGatekeeper.model` must use `<provider>/<model>:<effort>`. When enabled, assistant bash output at `<= 4096` tokens passes through directly, output between `4097` and `12288` tokens is reviewed by the configured gatekeeper model, and output above `12288` tokens is always gated to a 2,048-token preview. Explicit `maxOutputTokens` bypasses the gatekeeper and uses the requested budget directly.
...
- **Config levels**: `.tau/config.json` files are discovered from cwd up to home (or filesystem root if cwd is outside home). The global level is included only when cwd is under home. Scalars use most-specific wins; `apiKeys`, `sandbox`, `bashOutputGatekeeper`, `modelSystemNotices`, and `async.client` merge per field; `bashCommands` merge by `id` and run from the config level root (directory containing `.tau`, or home for the global config); `agentContextFiles` are additive.
