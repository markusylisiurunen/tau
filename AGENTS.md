...
Intermediate system messages use the strict plain-text contract in `src/protocol/system_message.ts`, with required typed, versioned metadata. They are distinct from the initial persona/base prompt, durable and hidden from the normal TUI, but recorded as system entries in searchable history without Tau metadata. Commit them through the awaited `AgentRuntime` event path at idle boundaries. Strip their metadata for model calls, preserve it for recovery and forks, and never promote user-authored tags into native system instructions. Compaction summarizes ordinary system history and removes metadata-marked obsolete continuation instructions; it does not alter the separate base prompt. Sections and tool mutations are unsupported.
...
When a user explicitly requests a Sol, Terra, or Luna subagent without a reasoning effort, use `openai-codex/gpt-6-sol:high` for Sol, `openai-codex/gpt-5.6-terra:high` for Terra, and `openai-codex/gpt-6-luna:high` for Luna. When a user explicitly requests a GPT-6 Astra subagent without a reasoning effort, use `openai-codex/gpt-6-astra:medium`. Otherwise omit a launch override unless the user requests one.
