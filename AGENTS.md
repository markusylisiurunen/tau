...
**Built-in**: Claude Opus 4.6 (`opus-4.6-chat`, `opus-4.6-coder`), Claude Opus 4.7 (`opus-4.7-chat`, `opus-4.7-coder`), GPT-5.3-Codex (`gpt-5.3-codex`), GPT-5.4 (`gpt-5.4-chat`, `gpt-5.4-coder`), GPT-5.3-Codex ChatGPT (`gpt-5.3-codex-chatgpt`), GPT-5.4 ChatGPT (`gpt-5.4-chatgpt-chat`, `gpt-5.4-chatgpt-coder`), GPT-5.3-Codex Fast ChatGPT (`gpt-5.3-codex-fast-chatgpt`), GPT-5.4 Fast ChatGPT (`gpt-5.4-chatgpt-fast-chat`, `gpt-5.4-chatgpt-fast-coder`), Gemini 3.1 Pro (`gemini-3.1-pro-chat`), Gemini 3 Flash (`gemini-3-flash-chat`). Optional GPT-5.5 built-ins (`gpt-5.5-chat`, `gpt-5.5-coder`, `gpt-5.5-chatgpt-chat`, `gpt-5.5-chatgpt-coder`, `gpt-5.5-chatgpt-fast-chat`, `gpt-5.5-chatgpt-fast-coder`) appear when `openai:gpt-5.5` and/or `openai-codex:gpt-5.5` are present in the merged model catalog, including `models.json`. Built-in personas include the **default** subagent (general-purpose, trigger: explicit) unless it is explicitly disabled. Built-in personas have `skills: "*"` to enable all discovered skills by default. See trigger sensitivity below for how subagent and skill activation is controlled.
