...
- **Global**: `~/.config/tau/config.json` (API keys, `defaultPersona`, `defaultRisk`, `disableBuiltinPersonas`, `disableBuiltinThemes`, `defaultTheme`, `diffTool`, `bashCommands`, `agentContextFiles`, `subagents`, `autoCompact`, `modelSystemNotices`, `async`). This level is only included when cwd is inside home.
