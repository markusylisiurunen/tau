...
- **Code mode** (`src/core/tools/code_mode.ts`, `src/core/tools/web.ts`, `src/core/static/code_mode/web/`): Generic code-tool lifecycle plus the Exa-backed `web` implementation. Generated JavaScript runs in a host-owned SES compartment with only bounded `web`, `docs`, and console capabilities; the trusted parent retains provider credentials, executes web requests, and returns stdout/stderr rather than JavaScript return values
