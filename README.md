...
Model `bash` tool calls, `!`/`!!`, and `session.exec` each run in a fresh, non-interactive login Bash belonging to the session execution environment. Tau sets `HOME` to the execution environment home, so Bash reads `/etc/profile` and then the first available user login file (`~/.bash_profile`, `~/.bash_login`, or `~/.profile`). Bash also reads the file named by an inherited `BASH_ENV` when set; otherwise `.bashrc` is loaded only when the login configuration sources it. Commands start from the backend's target-side environment and apply explicit execution-environment overrides; the local backend filters sensitive variables inherited from the Tau host. Shell state such as `cd`, exports, aliases, functions, and `nvm use` does not persist between calls. Tau-controlled machine operations use direct argv execution in the same target environment, without loading Bash startup files or parsing profile output.
