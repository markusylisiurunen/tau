...
the `modelSystemNotices` field maps `<provider>/<model>` to a notice string. provider ids must be known and model ids are exact/case-sensitive against the merged configured model catalog (built-in + layered `models.json`). tau prepends the notice as a `<system>...</system>` block to main-session user messages and sub-agent prompts, regardless of persona id. notices do not apply to ephemeral agents or maintenance model calls such as compaction and sampling.
