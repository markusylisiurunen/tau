...
Telegram config defines `bots`, `projects`, `workspaceRoot`, optional `systemMessage`, and optional `maxSessions`. repositories use persistent bare caches at `<workspaceRoot>-repo-cache/<projectId>.git`; active session records are persisted at `<workspaceRoot>-sessions.json`, their workspaces survive runner restarts, and unreferenced workspace entries are removed on startup. Tau reconnects preserved sessions to their corresponding snapshots. `bootstrapCommands` block initial readiness, and `backgroundBootstrapCommands` run after initial or recovered readiness. for config details, see [docs/telegram.md](docs/telegram.md).
