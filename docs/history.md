...
The tool progressively discloses its API documentation. Before using the API, the documentation must be visible in the conversation context; if it is not, one documentation-only call prints `docs` and a later call uses the documented API. Do not reload the documentation while it remains visible. This page does not duplicate those signatures or response limits. The documentation covers bounded chronological projections and targeted inspection for cases where a historical session is known but the relevant entry is not, avoiding repeated guessed searches and complete payload dumps.
