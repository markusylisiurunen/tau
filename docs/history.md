...
The tool is not the preferred recovery path for context removed from the current session by automatic compaction. When the compaction continuation supplies execution-environment archive paths, the agent should search and read those files instead. History is a separate host-owned collection and may be stale, truncated, unavailable, or remotely replicated; its transcript entry ids do not define archive lookup.

The tool owns its progressively disclosed API documentation. On the first history call for a task, the agent must print and read `docs`, then use the documented API in a later call. This page does not duplicate those signatures or response limits. When a historical session is already known but the relevant entry is not, the guidance suggests a bounded chronological overview followed by targeted inspection instead of repeated guessed searches or complete payload dumps. It includes adaptable examples for projecting a compact page and scanning transcript pages in code while printing only selected entries; the projection remains task-dependent rather than using a required format.
