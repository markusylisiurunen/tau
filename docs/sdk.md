...
`files.write(name, content)` atomically writes UTF-8 text and returns its absolute execution-environment path and byte count. `files.read(name)` reads the current real file, including changes made by Bash. `files.list()` returns bounded name, path, and size metadata, while `files.remove(name)` reclaims a file. Names are single basenames, and each agent's deterministic temporary directory is limited to 128 files and 64 MiB total. The directory is shared across code-mode tools and later calls by that agent, but isolated from other agents. Its contents live only as long as the execution environment retains its temporary filesystem; the session snapshot does not store them.
