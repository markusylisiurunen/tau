...
Before replacing context, Tau makes a best-effort archive in the execution environment’s temporary directory. Each automatic compaction adds a numbered `.txt` and `.json` pair under a directory isolated by agent id. The text file is convenient for bounded search and truncates large tool results; the JSON pair retains the archived content without those tool-result truncations, excluding assistant thinking. A private `README.md` in the same directory documents the formats, lifecycle, and adaptable lookup examples. The continuation message gives the agent all three exact paths when archiving succeeds.

For details removed from the current session by automatic compaction, these execution-environment files are preferred over the `history` tool. The tool queries a separate host-owned transcript collection, which may be remotely replicated, stale, truncated, or unavailable. Its transcript entry ids are not the archive lookup contract.

The continuation requires the full `README.md` contents to be present in current model context before work continues, even when no archive lookup is planned. It skips the read only when the guide is already visible in full, such as after an earlier compaction in the same context. Detailed schemas and examples live in that file, while the continuation retains the important defaults: prefer these files over history, keep reads bounded, use direct evidence when available, consider a concise overview before drill-down when it is not, and inspect earlier numbered pairs when necessary. Tau does not generate or require a fixed summary projection.
