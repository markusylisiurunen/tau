...
With `background: true`, `bash` returns a session-owned job ID after launch rather than waiting for exit. Launch does not establish application readiness. Background jobs have no default execution deadline; an explicit `timeout` still stops the command. They keep running across turns and interruptions. Main agents, subagents, and ephemeral threads share the session's job registry.
