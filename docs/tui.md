...
If transcription fails, Tau retains the local WAV and reports its path. Run `/listen retry` to transcribe the same audio again with the current provider and conversation context, or `/listen discard` to delete it. A replacement recording deletes the retained file only after the new capture starts producing audio. Tau keeps at most one failed recording, and exiting leaves that file at the reported path for manual recovery. Gemini transcription makes one request to Gemini 3.7 Flash. OpenAI live transcription sends the recent context to GPT-5.6 Luna to extract spelling hints while recording begins, then streams the buffered and subsequent audio to `gpt-live-transcribe` with English and Finnish language hints; hint extraction failure does not fail transcription. OpenAI uploads a retained recording with its recent context to `gpt-transcribe` on retry.
