...
`/listen` and Ctrl+Y are currently macOS-only. Recording uses local `ffmpeg` with the AVFoundation audio input and stops when Ctrl+Y is pressed again, when Escape is pressed, or after 20 minutes. Startup fails if the microphone produces no audio within 15 seconds. A manually stopped transcript is inserted at the cursor for review. Reaching the 20-minute limit stops recording and submits the transcript automatically.
