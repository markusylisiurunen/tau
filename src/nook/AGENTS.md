...
- Setup and destroy are CLI-only infrastructure flows. The model tool must operate only an already configured Nook target. Generated code receives no credentials, filesystem, process, environment, network, import, timer, or raw request authority; the host parent owns authenticated Nook HTTP and uses the generic execution backend for agent-visible paths.
