    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "void client.sessions.observe('session-1').then((session) => session.execProcess(['git', 'status'], { env: { HOME: '/home/user' } }));",
          "void client.sessions.observe('session-1').then((session) => session.readFile('/repo/file', { maxBytes: 1024 }));",
          "void client.sessions.observe('session-1').then((session) => session.writeFile('/repo/file', Buffer.from('file')));",
